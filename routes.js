...
module.exports = function(app){
	...
	app.get('/', function(req, res){
		...
		photos.find({}, function(err, all_photos){
			...
			users.find({ip: req.headers['x-forwarded-for'].split(',')[0]}, function(err, u){
	...
	function vote(req, res){
		...
		photos.find({ name: req.body.photo }, function(err, found){
			...
			if(found.length == 1){
				...
				users.update({ip: req.headers['x-forwarded-for'].split(',')[0]}, { $addToSet: { votes: found[0]._id}}, function(){
					res.redirect('../');
				});
